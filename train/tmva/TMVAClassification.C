                       ...
                       std::string outputname, float ptmin, float ptmax, std::string mymethod, std::string stage)
{
  ...
  if (mymethod != "") {
    ...
    for (auto& m : methods) {
      if (Use.find(m) != Use.end()) {
        Use[m] = 1;
        std::cout <<"==> " << __FUNCTION__ << ": Registered method " << m << std::endl; 
      } else {
        std::cout << "==> Abort " << __FUNCTION__ << ": error: unknown method " << m << "." << std::endl;
        continue;
      }
    }
  }
  ...
  auto* inputS = TFile::Open(inputSname.c_str());
  auto* inputB = TFile::Open(inputBname.c_str());
  ...
  auto* background = (TTree*)inputB->Get("Tree");
  auto* signal = (TTree*)inputS->Get("Tree");
 
  // Create a ROOT output file where TMVA will store ntuples, histograms, etc.
  //// TString outfileName( "TMVA.root" );
  //// TFile* outputFile = TFile::Open( outfileName, "RECREATE" );
  xjjroot::mkdir(outfname.c_str());
  auto* outf = TFile::Open(outfname.c_str(), "RECREATE");
